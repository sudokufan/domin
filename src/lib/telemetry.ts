  ...
  switch (telemetry.kind) {
    case "printer":
      return [
        { label: "Chamber temp", value: `${round(telemetry.chamberTempC)} °C` },
        {
          ...
          value: `${Math.round(telemetry.buildProgressPct)} %`,
        },
        {
          label: "Material remaining",
          value: `${Math.round(telemetry.materialRemainingPct)} %`,
        },
    ...
    case "lathe":
      return [
        ...
        {
          label: "Coolant temp",
          value: `${round(telemetry.coolantTempC, 1)} °C`,
        },
      ];
    case "test-rig":
      return [
        ...
        {
          label: "Result",
          value: telemetry.testResult
            ? telemetry.testResult.toUpperCase()
            : "In progress",
        },
        {
          label: "Inlet pressure",
          value: `${round(telemetry.inletPressureBar)} bar`,
        },
        {
          label: "Outlet pressure",
          value: `${round(telemetry.outletPressureBar)} bar`,
        },
        {
          label: "Flow rate",
          value: `${round(telemetry.flowRateLpm, 1)} L/min`,
        },
    ...
    case "marker":
      return [
        {
          label: "Parts marked (shift)",
          value: round(telemetry.partsMarkedShift),
        },
      ];
    case "shipping":
      return [
        {
          label: "Parts dispatched (shift)",
          value: round(telemetry.partsDispatchedShift),
        },
      ];
    case "honing":
      return [];
